...
//				}
//				
//				if((nHour >= sHour && nMinute >= sMinute) && (nHour <= eHour && nMinute <= eMinute)) {
//					//알림안함(매너모드)
//					Log.e("", "");
//				}
//				else {
...
//			}
			...
			//					mListItems.add(item);
			//				}
			mAlramBadge = (TextView)findViewById(R.id.noreadcnt);
			...
			mLogoBtn = (ImageButton) findViewById(R.id.title_logo_btn);
			...
			mLogoBtn.setOnClickListener(new OnClickListener() {
				@Override
				public void onClick(View v) {
					...
					if(mMainReportLayout.getVisibility() == View.VISIBLE) {
						mMainReportLayout.setVisibility(View.GONE);
					}
...
//					Toast.makeText(this, "우체통 상세화면 으로 이동 테스트 MSG_TYPE 1~8", Toast.LENGTH_SHORT).show();
					intent = new Intent(S00013.this, SC010Detail.class);
					intent.putExtra("seq",pushVo.getMsgNo()+"");
...
//					Toast.makeText(this, "본문글 상세화면 으로 이동 테스트 MSG_TYPE 10~14", Toast.LENGTH_SHORT).show();
					intent = new Intent(S00013.this, S00113.class);
					intent.putExtra("seq", pushVo.getContentsNo());
					intent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
					startActivity(intent);
...
//					Toast.makeText(this, "공지사항 상세화면 으로 이동 테스트 MSG_TYPE 16", Toast.LENGTH_SHORT).show();
					intent = new Intent(S00013.this, S00114.class);
					intent.putExtra("seq",pushVo.getContentsNo());
					intent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
					startActivity(intent);
				}
	...
	protected void onResume() {
			...
			public void onResult(boolean result, String resultData) {
				try {
					if(result){ /// 종료하면 여기로 탑니다
						...
						if(object != null){
							mAlarmCnt = object.getInt("noReadAlarm");
						}
					}else{}	
				} catch (Exception e) {
		...
		if(Global.mMainRefreshProfile == true) {
			pageCnt = 1;
			requestBoardList(1, mSearchText);
			Global.mMainRefreshProfile = false;
		}
	}
	...
	public void boardReportCallBack(String url, JSONObject jsonObject, AjaxStatus status) throws JSONException {
		try {
			...
			if(result) {
				...
				mListItems.get(mSelectPosition).setIsReport(1);
				mListAdapter.notifyDataSetChanged();
				mReportKind = -1;
				mReportUserId = "";
				mReportSeq = -1;
