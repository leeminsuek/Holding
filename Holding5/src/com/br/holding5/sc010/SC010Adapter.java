	...
	public View getView(int pos, View v, ViewGroup arg2) {
		...
		try {
			if(v == null)
			{
				...
				v.setTag(R.id.iv_s00120_item_profile_mask, v.findViewById(R.id.iv_s00120_item_profile_mask));
			}
			...
			else
			{
				...
				final ImageView maskProfile = (ImageView)v.getTag(R.id.iv_s00120_item_profile_mask);
				...
				if(lvl == 1){
					...
					maskProfile.setImageResource(R.drawable.prf_box_bg);
				}
				else{
					...
					if(phtchk.length() > 0){
						Log.d("SC010Adapter", "phtchk = " + phtchk);
						String url="";
						if(ITEM.get("happy_photo").toString().startsWith("http")) {
							url = ITEM.get("happy_photo").toString();
						}
						else {
							url = Global.host + ITEM.get("happy_photo").toString();
						}
						final String simgUrl =  url;
...
//						ivProfile.setImageDrawable(res.getDrawable(R.drawable.prf_box_bg));
					...
					if(lvl == 2) {
						maskProfile.setImageResource(R.drawable.happy_prf_bg_01);
					}
					else if(lvl == 3) {
						maskProfile.setImageResource(R.drawable.happy_prf_bg_02);
					}
					else if(lvl == 4) {
						maskProfile.setImageResource(R.drawable.happy_prf_bg_03);
					}
//					maskProfile.setImageResource(R.drawable.ma)
				}
				...
				tvWriterTime.setText(com.br.holding5.ms.Common.CreateDataWithCheck(sWriteTime));
