...
public class SC004_S11115 extends BaseActivity{
	...
	private TextView textView_ment;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			setContentView(R.layout.sc004_s11115);
			...
			mBoardContentAddBtn.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if(mBoardContentEdit.getText().toString().equals("")) {
						BrUtilManager.getInstance().showToast(SC004_S11115.this, "내용을 입력해 주세요.");
					}
					else {
			...
			mPictureAddBtn.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if(MAX_SELECT_COUNT - mSelectImgCount <= 0) {
						...
						Intent intent = new Intent(SC004_S11115.this,CustomGallery.class);
			...
			mAudioAddBtn.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(!mSelectAudioPath.equals("")) {
						BrUtilManager.getInstance().showToast(SC004_S11115.this, "오디오 파일은 한개만 선택 할 수 있습니다.");
						return;
					}
			...
			mCaptureVideoAddBtn.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(!mSelectVideoPath.equals("")) {
						BrUtilManager.getInstance().showToast(SC004_S11115.this, "비디오 파일은 한개만 선택 할 수 있습니다.");
						return;
					}
		...
		try {
			AsyncTask<Void, Void, String> loadImage = new AsyncTask<Void, Void, String>() {
				...
				protected String doInBackground(Void... params) {
					...
					boolean flag = BrImageUtilManager.getInstance().saveOutput(SC004_S11115.this,mVideoThumbnail,getImageUri(testfile.getAbsolutePath()));
...
//							if(!mPictureArr.get(j).isPictureYn()) {
						...
						del.setImage(mPictureBitmapArr.get(i), 1, false);
...
//			imageLayout.set_photoUrl(path);
	...
	private void requestGetDetail(int seq) {
		try {
			...
			aquery.progress(dialog).ajax(Global.HAPPYIN_HOPEMESSAGE, params, JSONObject.class, this, "boardDetailCallBack");			
		} catch (Exception e) {
			WriteFileLog.writeException(e);
		}
...
//			params.put("religion", (mSelectGroup3));
			...
			aquery.progress(dialog).ajax(Global.HAPPYIN_INSERTMESSAGE, params, JSONObject.class, this, "boardAddCallBack");
		} catch (Exception e) {
			WriteFileLog.writeException(e);
		}
